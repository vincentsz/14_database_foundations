...
For this exercise set, we will be working with the DDL statements to create and modify tables in a database 
that tracks planets around stars other than our Sun. To get started, first create a postgresql database named "extrasolar", 
...
stars table
    ...
    distance: the distance in light years from Earth. Define this as a whole number (e.g., 1, 2, 3, etc) that must be non-null 
    and greater than 0.
...
All of them deal with how to alter a database's schema when the database already contains data; 
this sometimes requires additional work beyond just the statement provided in the main problem solution.
